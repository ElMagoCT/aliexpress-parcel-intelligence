...
export function OrdersView({ onSelect }: { onSelect: (id: string) => void }) {
  ...
  const rows = useMemo(() => {
    ...
    return orders.filter((o) => !trip || o.checkoutGroup === trip).filter((o) => flt === 'all' || o.status === flt).filter((o) => !f || o.orderId.includes(f) || o.sellerName?.toLowerCase().includes(f) || (itemsBy.get(o.orderId) ?? []).some((i) => i.title.toLowerCase().includes(f)))
      .sort((a, b) => {
  ...
  }, [orders, q, flt, trip, sort, itemsBy, settings]);
