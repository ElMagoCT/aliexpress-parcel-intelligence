...
const RULES: Rule[] = [
  ...
  { m: 'DELIVERED', re: /\bdelivered\b(?! (?:to (?:the )?(?:local|destination|carrier|courier|customs|airline|warehouse|sorting|next|agent|forwarder)))|successfully delivered|delivery (?:completed|successful|success|confirmed)|\bsigned\b(?! for by carrier)|signature (?:obtained|received)|received by (?:the )?(?:recipient|customer|addressee|consignee)|collected by (?:the )?(?:recipient|customer|addressee)|picked up by (?:the )?(?:recipient|customer|addressee)|left (?:in|at|with) (?:the )?[\\w' ]{0,24}(?:mailbox|mail ?room|front ?door|front ?porch|porch|reception|desk|neighbou?r|safe place|parcel box|letterbox|security gate|gate|garage|carport|lobby|side door|back door|doorstep)|parcel (?:has been )?collected|delivery to (?:the )?(?:mailbox|address) completed|item delivered|package (?:was )?delivered/i },
